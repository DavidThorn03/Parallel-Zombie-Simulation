//
//This function counts the number of Susceptible and infected neighbours that a Zombie
...
int countSusceptibleNeighbours(int row,
  ...
  for(i=row-1; i<=row+1; i++)
	...
	if(current[(i+ROWS)%ROWS][(j+COLS)%COLS].state == 'S' || current[(i+ROWS)%ROWS][(j+COLS)%COLS].state == 'I')
