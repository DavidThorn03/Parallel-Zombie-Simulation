...
int main(){
  ...
  //The quantity of S, I, Z, R, D could be large numbers: make them long ints
  unsigned long numS = 0;
  unsigned long numI = 0;
  ...
  for(day=1; day <= TOTAL_DAYS; day++){
	...
	case 'I':
	  ...
	  decide_I_to_Z(row, col, &numI, &numZ, current, future);
	  ...
	  if(current[row][col].stateChange == 'N'){
	    ...
	    decide_I_to_D(row, col, &numI, &numD, current, future);
	  }
