...
void *child_thread(void *args){
	...
	case 'S':
	  ...
	  decide_S_to_IorR(row, col, num_Zombie_Neighbours,	\
			   &thread_numS, &thread_numI, &thread_numR,			\
			   current, future);
	...
	case 'I':
	  ...
	  decide_I_to_Z(row, col, &thread_numI, &thread_numZ, current, future);

	  if(current[row][col].stateChange == 'N'){

	    #if DEBUG_LEVEL > 1
	    printf("main: Infected didnt turn");
	    #endif
	    
	    decide_I_to_D(row, col, &thread_numI, &thread_numD, current, future);
	  }
	...
	case 'Z':
	  ...
	  //number of Susceptible and infected neighbours to do so, and then based
	...
	//Output numerical data to the daySIZRD.dat file for the
        //day just completed.
       	outputDaySIZRD(fp_daySIZRD, day, numS, numI, numZ, numR, numD);
